...
import { Play, CheckCircle, XCircle, AlertCircle, Clock, Loader2, Zap } from 'lucide-react'
...
export function RunPanel() {
  ...
  const baseParallel = state.draft?.maxParallelChunks || 3
  const parallelMultiplier = state.draft?.parallelMultiplier || 1
  const effectiveParallel = activeRun?.config.maxParallelChunks || Math.min(100, baseParallel * parallelMultiplier)
      ...
      <CardContent className="space-y-3 sm:space-y-4">
        <div className="space-y-1.5 sm:space-y-2">
          ...
          {totalChunks > 0 && (
            <div className="text-[10px] sm:text-xs text-muted-foreground text-center space-y-0.5">
              <p>Chunk {progress.completedChunks} of {totalChunks} completed</p>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2 sm:gap-3 text-xs sm:text-sm">
          ...
          <div className="bg-muted/50 rounded p-2 sm:p-3">
            ...
            <p className="font-medium">{effectiveParallel} chunks</p>
          </div>
          <div className="bg-muted/50 rounded p-2 sm:p-3">
        ...
        <div className="border-t pt-4">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-xs text-muted-foreground ml-auto">
              Base: {baseParallel} chunks x {parallelMultiplier}x = <strong>{effectiveParallel} chunks</strong>
            </span>
          </div>
              ...
              <button
                ...
                onClick={() => {
                  if (!state.isTranslating && state.draft) {
                    dispatch({
                      type: 'SET_DRAFT',
                      payload: { ...state.draft, parallelMultiplier: mult },
                    })
                  }
                }}
          ...
          <div className="border-t pt-4">
            <div className="flex items-center justify-between mb-2">
              ...
              <span className="text-xs text-muted-foreground">
                {activeRun.chunks.filter((chunk) => chunk.status === 'success' || chunk.status === 'truncated').length} / {activeRun.chunks.length} done
              </span>
            </div>
